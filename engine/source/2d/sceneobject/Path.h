...
// t2dPath
//---------------------------------------------------------------------------------------------
class t2dPath : public SceneObject
{
...
public:
   ...
   virtual void preIntegrate(const F32 sceneTime, const F32 elapsedTime, DebugStats *pDebugStats);
   // For editor support. Resets pathed objects.
   virtual void integrateObject(const F32 sceneTime, const F32 elapsedTime, DebugStats* pDebugStats);
   ...
   virtual void sceneRender(const SceneRenderState* pSceneRenderState, const SceneRenderRequest* pSceneRenderRequest, BatchRender* pBatchRenderer) override;
   // Node management.
   ...
   void setSpeed(SceneObject* object, F32 speed)
   {
      PathedObject* pathedObject = getPathedObject(object);
      if (pathedObject) pathedObject->setSpeed(speed, true);
   }
   void setMoveForward(SceneObject* object, bool forward)
   {
      setDirection(object, forward ? 1 : -1);
   }
   void setDirection(SceneObject* object, S32 direction)
   {
      PathedObject* pathedObject = getPathedObject(object);
      if (pathedObject) pathedObject->setStartDirection(direction, true);
   }
   void setOrient(SceneObject* object, bool orient)
   {
      PathedObject* pathedObject = getPathedObject(object);
      if (pathedObject) pathedObject->setOrientToPath(orient, true);
   }
   void setRotationOffset(SceneObject* object, F32 offset)
   {
      PathedObject* pathedObject = getPathedObject(object);
      if (pathedObject) pathedObject->setRotationOffset(offset, true);
   }
   void setLoops(SceneObject* object, S32 loops)
   {
      PathedObject* pathedObject = getPathedObject(object);
      if (pathedObject) pathedObject->setTotalLoops(loops, true);
   }
   void setFollowMode(SceneObject* object, ePathMode followMode)
   {
      PathedObject* pathedObject = getPathedObject(object);
      if (pathedObject) pathedObject->setPathMode(followMode, true);
   }
   ...
   F32 getRotationOffset(SceneObject* object)
   {
      PathedObject* pathedObject = getPathedObject(object);
