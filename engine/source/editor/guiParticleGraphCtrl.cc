...
GuiParticleGraphCtrl::GuiParticleGraphCtrl()
{
   ...
   for(int i = 0; i < MaxPlots; i++)
   {
	   ...
	   mPlots[i].mGraphScale = 0.05;
   }

   mPlots[0].mGraphColor = ColorF(1.0, 0.2, 0.2);
   mPlots[1].mGraphColor = ColorF(1.0, 0.5, 0.5);
   ...
   mPlots[6].mGraphColor = ColorF(0.5, 0.5, 0.5);
   mPlots[7].mGraphColor = ColorF(0.5, 0.0, 0.0);
   mPlots[8].mGraphColor = ColorF(0.0, 0.5, 0.0);
   mPlots[9].mGraphColor = ColorF(0.0, 0.0, 0.5);
   mPlots[10].mGraphColor = ColorF(0.0, 0.5, 0.5);
   mPlots[11].mGraphColor = ColorF(0.25, 0.25, 0.25);
   mPlots[12].mGraphColor = ColorF(0.5, 0.5, 0.5);
   mPlots[13].mGraphColor = ColorF(0.5, 0.0, 0.0);
   mPlots[14].mGraphColor = ColorF(0.0, 0.5, 0.0);
   mPlots[15].mGraphColor = ColorF(0.0, 0.0, 0.5);
   mPlots[16].mGraphColor = ColorF(0.0, 0.5, 0.5);
   mPlots[17].mGraphColor = ColorF(0.25, 0.25, 0.25);
   mPlots[18].mGraphColor = ColorF(1.0, 0.2, 0.2);
   mPlots[19].mGraphColor = ColorF(1.0, 0.5, 0.5);
   mPlots[20].mGraphColor = ColorF(0.0, 1.0, 0.0);
   mPlots[21].mGraphColor = ColorF(0.0, 0.0, 1.0);
   mPlots[22].mGraphColor = ColorF(0.0, 1.0, 1.0);
   mPlots[23].mGraphColor = ColorF(0.0, 0.0, 0.0);
   mPlots[24].mGraphColor = ColorF(0.5, 0.5, 0.5);
   mPlots[25].mGraphColor = ColorF(0.5, 0.0, 0.0);
   mPlots[26].mGraphColor = ColorF(0.0, 0.5, 0.0);
   mPlots[27].mGraphColor = ColorF(0.0, 0.0, 0.5);
   mPlots[28].mGraphColor = ColorF(1.0, 0.0, 0.0);
   mPlots[29].mGraphColor = ColorF(0.0, 1.0, 0.0);
   mPlots[30].mGraphColor = ColorF(0.0, 0.0, 1.0);
   mPlots[31].mGraphColor = ColorF(0.5, 0.0, 0.0);
	...
	for (int k = 0; k < MaxPlots; k++)
	{
		...
		if((mPlots[k].mGraphType == Point) || (mPlots[k].mGraphType == Polyline))
		{
         S32 posX;
            ...
            if(sample > 0)
				{
				   dglDrawLine( lastPosX, lastPosY , posX, posY , mPlots[k].mGraphColor );
				} else
				{
            ...
            // draw the seleciton nut
               ...
               if((k == mSelectedPlot && sample == mLastSelectedPoint) || (mRenderAllPoints == true))
                  ...
                  S32 textWidth = 2;
                  if (!font.isNull())
                     textWidth = (S32)font->getStrWidth((const UTF8*)number);
				      textWidth /= 2;

				      if((((S32)posX - (textWidth/2)) < comparePos.x) || (((S32)posX - textWidth) <= 0))
				      {
                      posX += (textWidth/1.5);
				      } else if((posX + (textWidth * 1.8)) > (comparePos.x + getExtent().x) || ((posX + textWidth) >= res.w))
				      {
                      posX -= (textWidth * 1.5);
				      }
                  ...
                  if (profile)
				         dglSetBitmapModulation(profile->mFontColor);
                  
                  if (!font.isNull())
                     dglDrawText(font, Point2I(posX, posY + 5) - Point2I(size >> 1, size), number);
				   }
				}
			}
		}
...
void GuiParticleGraphCtrl::insertPlotPoint(S32 plotID, S32 i, Point2F v)
{
   ...
   // Call Scripts.
   Con::executef(this, 3, "onPlotPointInserted", argBuffer[0], argBuffer[1]);
}
...
S32 GuiParticleGraphCtrl::changePlotPoint(S32 plotID, S32 i, Point2F v)
{
   ...
   // Call Scripts.
   Con::executef(this, 4, "onPlotPointRemoved", argBuffer[0], argBuffer[1], argBuffer[2]);
...
void GuiParticleGraphCtrl::setSelectedPlot(S32 plotID)
{
   ...
   // Call Scripts.
   Con::executef(this, 2, "onSetSelected", argBuffer);
}
...
void GuiParticleGraphCtrl::setSelectedPoint(S32 point)
{
   if(point != mSelectedPoint && point < mPlots[mSelectedPlot].mGraphData.size() && point >= 0)
   {
      ...
      // Call Scripts.
      Con::executef(this, 2, "onPlotPointSelected", argBuffer);
   }
}
...
void GuiParticleGraphCtrl::onMouseMove(const GuiEvent &event)
{
   ...
   // Call Scripts.
   Con::executef(this, 2, "onMouseMove", argBuffer);
}
...
void GuiParticleGraphCtrl::onMouseDown(const GuiEvent &event)
{
   ...
   if( hitNut != Point2I(-1,-1) )
   {
	  ...
	  } else
	  {
        ...
        // Call Scripts.
        Con::executef(this, 2, "onPlotPointSelectedMouseDown", argBuffer);
	  }
   ...
   {  
      ...
      // Call Scripts.
      Con::executef(this, 2, "onMouseDragged", argBuffer);
      ...
      if(mSelectedPoint == -1)
	   {
         ...
         // Call Scripts.
         Con::executef(this, 4, "onPlotPointAdded", argBuffer[0], argBuffer[1], argBuffer[2]);
	   } else
	   {
         // Argument Buffer.
         char argBuffer[4][32];
         ...
         // Call Scripts.
         Con::executef(this, 5, "onPlotPointChangedUp", argBuffer[0], argBuffer[1], argBuffer[2], argBuffer[3]);
	  }
   }
      ...
      if(point != -1)
	   {
         ...
         // Call Scripts.
         Con::executef(this, 4, "onPlotPointChangedMove", argBuffer[0], argBuffer[1], argBuffer[2]);
	   } else
	   {
...
bool GuiParticleGraphCtrl::renderGraphTooltip(Point2I cursorPos, StringTableEntry tooltip)
{
   ...
   dglDrawRectFill(rect, ColorI(mTooltipProfile->mFillColor.red, mTooltipProfile->mFillColor.green, mTooltipProfile->mFillColor.blue, 200) );
   ...
   dglDrawText( font, start + offset, tooltip, mProfile->mFontColors );
