...
//
// 2D Scene Object Selection Tool
//---------------------------------------------------------------------------------------------
...
static EnumTable::Enums displayRuleLookup[] =
                {
                { SelectionToolWidget::UnMountedOnly,    "Unmounted" },
                { SelectionToolWidget::MountedOnly,   "Mounted" },
...
bool LevelBuilderSelectionTool::onKeyDown(LevelBuilderSceneWindow* sceneWindow, const GuiEvent &event)
{
   ...
   UndoMoveAction* undoMove = new UndoMoveAction(mOwner, "Nudge Objects");
...
// onMouseMove
//---------------------------------------------------------------------------------------------
bool LevelBuilderSelectionTool::onMouseMove( LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus &mouseStatus )
{
   ...
   else
   {
      ...
      {
         if (mOwner->isAcquired(mouseStatus.pickList[i]))
         {
            if (i == (mouseStatus.pickList.size() - 1))
               mHoverObj = mouseStatus.pickList[0];
            else
               mHoverObj = mouseStatus.pickList[i + 1];
...
// onMouseDown
//---------------------------------------------------------------------------------------------
bool LevelBuilderSelectionTool::onMouseDown( LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus &mouseStatus )
{
   mAddUndo = false;
   mMouseDown = true;
   ...
   if (mOwner->getAcquiredObjectCount())
   {
      ...
      if( mSizingState != SizingNone )
      {
         ...
         if (mouseStatus.event.modifier & SI_ALT)
         {
            ...
            mStartRotation = mOwner->getAcquiredObjects().getRotation();
            mRotationVector = mouseStatus.mousePoint2D - mOwner->getAcquiredObjects().getPosition();
   ...
   {
      for (U32 i = 0; i < mouseStatus.pickList.size(); i++)
      {
         if (mOwner->isAcquired(mouseStatus.pickList[i]))
         {
...
// onMouseDragged
//---------------------------------------------------------------------------------------------
bool LevelBuilderSelectionTool::onMouseDragged( LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus &mouseStatus )
{
   LevelBuilderSceneEdit* mOwner = sceneWindow->getSceneEdit();
   if (((mMouseState == Selecting) || (mMouseState == SelectingWidget)) && (mouseStatus.dragRectNormal.extent.len() > 1))
   {
   ...
   else if (mMouseState == MovingSelection)
   {
      for (S32 i = 0; i < mOwner->getAcquiredObjectCount(); i++)
      {
         if ((mOwner->getAcquiredObject(i)->mpMountedTo) &&
             (!mOwner->isAcquired(mOwner->getAcquiredObject(i)->mpMountedTo)))
         {
            mCantMove = true;
            return false;
         }
   ...
   else if (mMouseState == SizingSelection)
   {
      ...
      for (S32 i = 0; i < mOwner->getAcquiredObjectCount(); i++)
      {
         if ((mOwner->getAcquiredObject(i)->mpMountedTo) || (mOwner->getAcquiredObject(i)->getAttachedToPath()))
            uniform = true;
      }
      ...
      for (S32 i = 0; i < mOwner->getAcquiredObjectCount(); i++)
      {
         F32 rotation = mOwner->getAcquiredObject(i)->getRotation();
      ...
      scale(mOwner, mOwner->getAcquiredObjects().getSize(), mOwner->getAcquiredObjects().getPosition(), mouseStatus.mousePoint2D,
   ...
   {
      mAddUndo = true;
      F32 oldRotation = mOwner->getAcquiredObjects().getRotation();
      F32 newRotation;
      rotate(mOwner, mStartRotation, mRotationVector, mouseStatus.mousePoint2D - mOwner->getAcquiredObjects().getPosition(), newRotation);
      mOwner->getAcquiredObjects().rotate(mDegToRad(newRotation) - mDegToRad(oldRotation));
      mOwner->onObjectSpatialChanged();
   }
...
bool LevelBuilderSelectionTool::onMouseUp( LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus &mouseStatus )
{
   mMouseDown = false;
      ...
      else
      {
         ...
         for (S32 i = 0; i < mouseStatus.pickList.size(); i++)
         {
            if (mOwner->isAcquired(mouseStatus.pickList[i]))
            {
               ...
               if (i == (mouseStatus.pickList.size() - 1))
                  mOwner->requestAcquisition(mouseStatus.pickList[0]);
               else
                  mOwner->requestAcquisition(mouseStatus.pickList[i + 1]);
   ...
   // Nothing to do for sizing and rotating.
   else if (mMouseState == SizingSelection)
   {
      mMouseState = Selecting;
      if ((mAddUndo) && (mCurrentUndo))
   ...
   else if (mMouseState == DragSelecting)
   {
      ...
      for (S32 i = 0; i < mouseStatus.dragPickList.size(); i++)
      {
         ...
         if (!mOwner->isAcquired(mouseStatus.dragPickList[i]))
         {
            mOwner->requestAcquisition(mouseStatus.dragPickList[i]);
            selectionChanged = true;
         }
         else if (ctrl && !shift)
         {
            mOwner->clearAcquisition(mouseStatus.dragPickList[i]);
            selectionChanged = true;
         }
      }
...
void LevelBuilderSelectionTool::checkSelectedHoverObj(LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus& mouseStatus)
{
   ...
   else
   {
      for (S32 i = 0; i < mouseStatus.pickList.size(); i++)
      {
         if (sceneWindow->getSceneEdit()->isAcquired(mouseStatus.pickList[i]))
         {
            setSelectedHoverObj(mouseStatus.pickList[i]);
            return;
         }
      }
   }
...
void LevelBuilderSelectionTool::onRenderGraph(LevelBuilderSceneWindow* sceneWindow)
{
   LevelBuilderSceneEdit* mOwner = sceneWindow->getSceneEdit();
   // Render Parent first
   ...
   if( mOwner->hasAcquiredObjects() )
   {
      ...
      for (U32 i = 0; i < mOwner->getAcquiredObjectCount(); i++)
      {
         ...
         // Draw Bounding Box
         dglDrawRect( objRect, mHoverOutlineColor );
      }
