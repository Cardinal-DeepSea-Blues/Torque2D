...
void LevelBuilderTextEditTool::editObject( SceneObject* object )
{
   ...
   // If it passed the isEditable test, we know it's a t2dTextObject.
   mTextObject = static_cast<t2dTextObject*>( object );
...
void LevelBuilderTextEditTool::finishEdit()
{
   ...
   // Reset Rotation
   mTextObject->setRotation( mRotation );
...
// Mouse Events
//-----------------------------------------------------------------------------
bool LevelBuilderTextEditTool::onMouseDown( LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus &mouseStatus )
{
   ...
   {
      RectF bounds = mTextObject->getWorldClipRectangle();
...
bool LevelBuilderTextEditTool::onMouseDragged(LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus &mouseStatus)
{
   ...
   if( mSizingState != SizingNone )
   {
      ...
      if( mTextObject->mpMountedTo || mTextObject->getAttachedToPath() || mouseStatus.event.modifier & SI_CTRL )
         uniform = true;

      // Shift scales with maintained aspect ratio.
      bool shiftScale = false;

      F32 rotation = mTextObject->getRotation();
...
bool LevelBuilderTextEditTool::onMouseUp(LevelBuilderSceneWindow* sceneWindow, const t2dEditMouseStatus &mouseStatus)
{
   if (!mTextObject || (sceneWindow != mSceneWindow))
      return false;
...
void LevelBuilderTextEditTool::onRenderGraph( LevelBuilderSceneWindow* sceneWindow )
{
   ...
   drawWordWrapSizingNuts( sceneWindow, mTextObject->getWorldClipRectangle() );
